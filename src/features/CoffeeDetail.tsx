  ...
  async function finalize(finalFlavor: FlavorProfile) {
    ...
    try {
      ...
      if (a.source === "local") {
        toast(
          a.fallbackReason
            ? `Claude unavailable — using built-in advice. ${a.fallbackReason}`
            : "Claude unavailable — using built-in advice."
        );
      }
    } finally {
  ...
  return (
    <section>
      <div className="row-between">
        ...
        {advice && (
          <span
            className={`status-pill ${advice.source === "claude" ? "ok" : "due"}`}
            title={advice.source === "claude" ? "Generated by Claude" : advice.fallbackReason || "Local fallback"}
          >
            {advice.source === "claude" ? "via Claude" : "built-in only"}
          </span>
        )}
      </div>
      {advice && (
          ...
          {advice.source === "local" && (
            <div
              ...
              style={{
                ...
                borderColor: "rgba(176, 74, 58, 0.25)",
              }}
            >
              <strong style={{ fontSize: 14, color: "#8a3a2e" }}>
                Bruna couldn't reach Claude for this shot
              </strong>
              <p className="muted" style={{ fontSize: 13, marginTop: 6, lineHeight: 1.45 }}>
                {advice.fallbackReason || "Advice below is from the built-in SCA method — usable, but not personalised to this coffee's roaster profile or your palate."}{" "}
                <a href="#/settings">Open Settings</a> to check your API key.
              </p>
            </div>
          )}
